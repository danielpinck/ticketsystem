...
function createTicket($db, $title, $description, $priority, $userId) {
    $stmt = $db->prepare("INSERT INTO tickets (title, description, priority, user_id) VALUES (:title, :description, :priority, :user_id)");
...
function getUserTickets($db, $userId) {
    $stmt = $db->prepare("SELECT * FROM tickets WHERE user_id = :user_id ORDER BY user_id DESC");
    $stmt->bindParam(':user_id', $userId);
    $stmt->execute();
